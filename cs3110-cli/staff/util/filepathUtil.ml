...
let get_subdirectories dir =
  if Sys.is_directory (strip_trailing_slash dir)
  then begin
    Array.fold_right (fun fname dirs ->
                      if Sys.is_directory (strip_trailing_slash fname)
                      then fname::dirs
                      else dirs)
                     (Sys.readdir dir)
                     []
    ...
    let msg = Printf.sprintf "'%s' is not a valid directory!" dir in
    failwith msg
  end
